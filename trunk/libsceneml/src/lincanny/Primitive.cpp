...
VERTEX3D::VERTEX3D():FEATURE(VERTEX_FEATURE){
	x = y = z = 0.0;
	adjacency_size = 0;  
	adjacency_max = 4;
}

VERTEX3D::VERTEX3D(double x, double y, double z):FEATURE(VERTEX_FEATURE){
	this->x = x;
	this->y = y;
	this->z = z;
	adjacency_size = 0;  
...
double VERTEX3D::dot(VERTEX3D v){
...
void VERTEX3D::transform(double* t, double &tx, double &ty, double &tz){
	tx = x*t[0] + y*t[4] + z*t[8] + t[12];
	ty = x*t[1] + y*t[5] + z*t[9] + t[13];
	tz = x*t[2] + y*t[6] + z*t[10] + t[14];
}

void VERTEX3D::transform(double* t, VERTEX3D& v){
...
void VERTEX3D::rotate(double* t, VERTEX3D& v){
	v.x = x*t[0] + y*t[4] + z*t[8];
	v.y = x*t[1] + y*t[5] + z*t[9];
	v.z = x*t[2] + y*t[6] + z*t[10];
}

void VERTEX3D::translate(double* t, double &tx, double &ty, double &tz){
	tx += t[12];
	ty += t[13];
	tz += t[14];
}
...
int FACE::getVertexIndex(VERTEX3D* v){
